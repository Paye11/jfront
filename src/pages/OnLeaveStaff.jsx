...
const OnLeaveStaff = () => {
  ...
  // Fetch all courts & on leave staff
  useEffect(() => {
  ...
  const handleDelete = async (staff) => {
    if (!staff._id) return toast.error("Cannot delete unsaved staff");
  ...
  return (
    <div className="p-4 sm:p-6 bg-gray-50 min-h-screen">
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-6 gap-3">
        <h2 className="text-lg sm:text-xl font-bold">Staff On Leave</h2>
      </div>
                ...
                ? staffList.map((s) => {
                    ...
                    return (
                      <tr key={s._id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-6 py-4">{s.area || "N/A"}</td>
                        <td className="px-6 py-4">{formatDate(s.leaveStartDate)}</td>
                        <td className="px-6 py-4">{formatDate(s.leaveEndDate)}</td>
                ...
                : !loading && (
                    <tr>
                      <td colSpan="11" className="px-6 py-8 text-center text-gray-500">
                        No staff members on leave found
                      </td>
                    </tr>
                  )}
            </tbody>
                  ...
                  return (
                    <div key={s._id} className="p-4">
                      ...
                      <p className="text-sm text-gray-600">
                        {courtInfo.type} - {courtInfo.name}
                      </p>
                      <p className="text-sm text-gray-600">{s.area || "N/A"}</p>
                      <p className="text-sm text-gray-600">
                        {formatDate(s.leaveStartDate)} → {formatDate(s.leaveEndDate)}
                      </p>
